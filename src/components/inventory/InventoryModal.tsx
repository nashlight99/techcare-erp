      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[92vh] flex flex-col">
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto">
          <div className="px-6 py-5 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="label">Marque compatible</label>
                <input className="input" placeholder="Apple, Samsung..." value={form.brand ?? ''} onChange={e => set('brand', e.target.value)} />
              </div>
              <div>
                <label className="label">Modèle compatible</label>
                <input className="input" placeholder="iPhone 14 Pro..." value={form.model ?? ''} onChange={e => set('model', e.target.value)} />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <input className="input" type="number" min="0" value={form.min_quantity} onChange={e => set('min_quantity', e.target.value)} />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input className="input" type="number" step="0.01" min="0" placeholder="0.00" value={form.unit_cost ?? ''} onChange={e => set('unit_cost', e.target.value)} />
              </div>
              <div>
                <label className="label">Prix de vente (€)</label>
                <input className="input" type="number" step="0.01" min="0" placeholder="0.00" value={form.sale_price ?? ''} onChange={e => set('sale_price', e.target.value)} />
              </div>
            </div>
